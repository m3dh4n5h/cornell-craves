-- Cornell Craves — apply ALL pending changes to reach migration 040.
-- Generated 2026-07-03. Paste this whole file into the Supabase SQL editor and Run.
--
-- PREREQUISITE: APPLY_PENDING_THROUGH_038.sql must already be applied (it
-- brought the database to 038). This file adds 039 + 040. Both parts are
-- idempotent, so re-running is safe. Wrapped in a transaction (all-or-nothing).
--
-- What 040 fixes (verified by 28 automated scenarios on a fresh database):
--   * "Approve once" is now durable per club: after one admin approval, every
--     future drop the club posts with that brand publishes immediately (or
--     autoposts), with no dead-end re-requests.
--   * The database itself now blocks publishing unapproved brands (the
--     template "Post" path used to skip the gate entirely).
--   * Approvals no longer "publish" already-expired listings into the void;
--     they become one-click postable drafts instead.
--   * New admin controls: per-club brand grants (revocable), full listings
--     moderation (hide/restore), held-drop counts on brand requests.
--   * New club dashboard stats RPC (revenue, pending payments, reservations).
--
-- After it succeeds, run the verification block at the very bottom (outside
-- the txn).

BEGIN;

-- ====================================================================
-- 039_post038_hardening.sql
-- ====================================================================
-- Re-affirm is_admin() to the real admin email and drop the legacy anon-
-- executable helpful-vote RPCs. Idempotent.
...
-- ====================================================================
-- 040_brand_workflow_integrated.sql
-- ====================================================================
...
select
  (select count(*) from information_schema.tables
     where table_name = 'club_brand_approvals') > 0                          as club_approvals_table,
  (select count(*) from pg_trigger
     where tgname = 'listings_brand_gate') > 0                               as brand_gate_trigger,
  pg_get_functiondef('public.request_brand(text)'::regprocedure)
     like '%is_brand_approved_for_club%'                                     as request_brand_updated,
  (select count(*) from pg_proc where proname = 'admin_listings') > 0        as admin_moderation_rpc,
  (select count(*) from pg_proc where proname = 'club_dashboard_stats') > 0  as club_stats_rpc;
